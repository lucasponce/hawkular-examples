...
function query_events() {
    ...
    curl -X GET -H "$HEADER_JSON" -H "$HEADER_TENANT" "$HAWKULAR_URL/events?categories=DEPLOYMENT"
    
    printf "\n\nQuery LOG events:\n\n"
    curl -X GET -H "$HEADER_JSON" -H "$HEADER_TENANT" "$HAWKULAR_URL/events?categories=LOG"

    printf "\n\nQuery appA,appB events:\n\n"
    curl -X GET -H "$HEADER_JSON" -H "$HEADER_TENANT" "$HAWKULAR_URL/events?tags=app|appA,app|appB"
